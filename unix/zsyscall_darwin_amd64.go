...
func Clonefileat(src_dirfd int, src string, dst_dirfd int, dst string, flags int) (err error) {
	var _p0 *byte
	_p0, err = BytePtrFromString(src)
	if err != nil {
		return
	...
	_, _, e1 := syscall_syscall6(funcPC(libc_clonefileat_trampoline), uintptr(src_dirfd), uintptr(unsafe.Pointer(_p0)), uintptr(dst_dirfd), uintptr(unsafe.Pointer(_p1)), uintptr(flags), 0)
	if e1 != 0 {
		err = errnoErr(e1)
	}
	return
...
func Fclonefileat(src_dirfd int, dst_dirfd int, dst string, flags int) (err error) {
	var _p0 *byte
	_p0, err = BytePtrFromString(dst)
	if err != nil {
		return
	}
	_, _, e1 := syscall_syscall6(funcPC(libc_fclonefileat_trampoline), uintptr(src_dirfd), uintptr(dst_dirfd), uintptr(unsafe.Pointer(_p0)), uintptr(flags), 0, 0)
	if e1 != 0 {
		err = errnoErr(e1)
	}
	return
